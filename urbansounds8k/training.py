    ...
    class ClearMLDataLoader(TorchDataset):
        def __init__(self, dataset_name, project_name, folder_filter):
            ...
            # Xóa một số hàng bằng cách lọc theo lớp, điều này sẽ tạo ra khoảng trống trong chỉ mục khung dữ liệu
            # (ví dụ: 57 sẽ không còn tồn tại nữa) vì vậy thiết lập chỉ mục lại để đưa nó trở lại phạm vi đầy đủ, nếu không sẽ gặp lỗi sau
            # khi chọn một hàng theo chỉ mục
    ...
    def train(model, epoch):
        ...
        for batch_idx, (_, inputs, labels) in enumerate(train_loader):
            ...
            if batch_idx % log_interval == 0:  # in số liệu thống kê đào tạo
            ...
            if batch_idx % debug_interval == 0:  # báo cáo hình ảnh gỡ lỗi mỗi đợt nhỏ "debug_interval"
    ...
    def test_model(model, epoch):
        ...
        with torch.no_grad():
            for idx, (sound_paths, inputs, labels) in enumerate(test_loader):
                ...
                if idx % debug_interval == 0:  # báo cáo hình ảnh gỡ lỗi mỗi đợt nhỏ "debug_interval"
